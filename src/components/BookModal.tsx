...
import { Star, BookmarkPlus, BookmarkCheck, ShoppingCart, Package, AlertCircle, ArrowLeft } from "lucide-react";
      ...
      <DialogContent className="max-w-3xl max-h-[95vh] md:max-h-[90vh] overflow-hidden p-0">
        ...
        <div className="md:hidden flex items-center gap-2 p-4 border-b border-border">
          <Button
            variant="ghost"
            size="icon"
            onClick={showReviews ? () => setShowReviews(false) : handleClose}
            className="shrink-0"
          >
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <h2 className="font-display text-lg font-semibold text-foreground truncate">
            {showReviews ? "Reviews" : book.name}
          </h2>
        </div>

        <div className="overflow-y-auto max-h-[calc(95vh-60px)] md:max-h-[90vh]">
          {/* Mobile: Show reviews or product details */}
          <div className={`md:hidden ${showReviews ? "block" : "hidden"}`}>
          ...
          <div className={`${showReviews ? "hidden md:grid" : "grid"} grid-cols-1 md:grid-cols-2 gap-0`}>
            {/* Left: Image */}
            ...
            {/* Desktop: Reviews column - hidden on mobile */}
            <div className="hidden md:flex md:col-span-2 flex-col border-t border-border p-6 max-h-[400px]">
              <ReviewSection
                itemId={book.id}
                itemType="book"
                itemName={book.name}
